...
Canvas{
    ...
    function draw()
    {
        ...
        function calculateXAxisSize(){
            ...
            scale = Math.ceil(valueHop/scaleFactor);
            //scale = 100.0;
        ...
        function drawScale(){
                ...
                ctx.fillRect(yAxisPosX, xAxisPosY + scaleLineWidth, xAxisLength -(valueHop/scale) + scaleLineWidth, - (calculatedScale.steps * scaleHop)-scaleLineWidth);
        ...
        if (completed && currentValue1 != maxYValue+1)
        {
            ...
            else if (pointData1.count < scale)
            {
                ...
                if (blankOut)
                {
                    ...
                    if (xPos(pointData1.count-1) + xPixels > end)
                        distance = end - xPos(pointData1.count-1);

                    if (imageBackground.source.toString().length > 0)
                        ctx.clearRect(xPos(pointData1.count-1), topY-datasetStrokeWidth, distance+2, (calculatedScale.steps * scaleHop) + 2*datasetStrokeWidth);
                    else
                    {
                        ctx.fillStyle = scaleBgColor;
                        ctx.fillRect(xPos(pointData1.count-1), xAxisPosY, distance, - (calculatedScale.steps * scaleHop) - scaleLineWidth);
                    }
                    if (scaleShowLabels)
                        updateTime();
                }
                ...
                var d = {'value': currentValue1};
                lastX1 = 0;
                lastY1 = pointData1.get(scale-1).value;
                ...
                if (imageBackground.source.toString().length > 0)
                    ctx.clearRect(xPos(lastX1)-2, xAxisPosY + scaleLineWidth, xPixels, - (calculatedScale.steps * scaleHop) - scaleLineWidth*2);
                else
                {
                    ctx.fillStyle = scaleBgColor;
                    ctx.fillRect(xPos(lastX1), xAxisPosY + scaleLineWidth, xPixels, - (calculatedScale.steps * scaleHop) - scaleLineWidth*2);
                }
                blankOut = true;
                var draw = drawLine(lastX1, lastY1, line1PenColor, pointData1);
                lastX1 = draw.lastX;
    ...
    onCurrentValue2Changed: {
        if (completed && currentValue2 != maxYValue+1)
        {
            ...
            else if (pointData2.count < scale)
            {
                ...
                var d = {'value': currentValue2};
                lastX2 = 0;
                lastY2 = pointData2.get(scale-1).value;
                pointData2.clear();
                pointData2.append(d);
                var draw = drawLine(lastX2, lastY2, line2PenColor, pointData2);
                lastX2 = draw.lastX;
    ...
    onCurrentValue3Changed: {
        if (completed && currentValue3 != maxYValue+1)
        {
            ...
            else if (pointData3.count < scale)
            {
                ...
                var d = {'value': currentValue3};
                lastX3 = 0;
                lastY3 = pointData3.get(scale-1).value;
                pointData3.clear();
                pointData3.append(d);
                var draw = drawLine(lastX3, lastY3, line3PenColor, pointData3);
                lastX3 = draw.lastX;
    ...
    onCurrentValue4Changed: {
        if (completed && currentValue4 != maxYValue+1)
        {
            ...
            else if (pointData4.count < scale)
            {
                ...
                var d = {'value': currentValue4};
                lastX4 = 0;
                lastY4 = pointData4.get(scale-1).value;
                pointData4.clear();
                pointData4.append(d);
                var draw = drawLine(lastX4, lastY4, line4PenColor, pointData4);
                lastX4 = draw.lastX;
